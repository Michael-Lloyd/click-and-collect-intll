...
let is_reversible_rule = function
    ...
    | ILL_With_left_1 -> false  (* With left 1 requires choice between left rules *)
    | ILL_With_left_2 -> false  (* With left 2 requires choice between left rules *)
    | ILL_With_right -> true    (* With right is reversible - must prove both conjuncts *)
    | ILL_Lollipop_left -> true (* Lollipop left is reversible *)
    | ILL_Axiom -> false        (* Axiom requires specific conditions *)
    | ILL_One_right -> false    (* One right requires empty context *)
    | ILL_One_left -> true      (* One left is reversible *)
